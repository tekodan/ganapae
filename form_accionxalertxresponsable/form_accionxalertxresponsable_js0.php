...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_accionxalertxresponsable"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
function sc_btn_Finalizar()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_Finalizar(); }, 500);
      return;
    }
    if (confirm("<?php echo html_entity_decode('Completado', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>"))
    {
        document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Finalizar?@?";
        document.F1.action = "./";
...
{ 
    ...
    Nm_Proc_Atualiz = true;
    do_ajax_form_accionxalertxresponsable_submit_form(y, z); 
} 
<?php
}
?>
...
function nm_mostra_doc(campo1, campo2, campo3)
{
    ...
    NovaJanela = window.open ("form_accionxalertxresponsable_doc.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&script_case_session=<?php echo session_id() ?>&nm_cod_doc=" + campo1 + "&nm_nome_doc=" + encodeURIComponent(campo2) + "&nm_cod_apl=" + campo3, "sc_name_download_iframe", "resizable, scrollbars");
}
